...
export async function loadFunStats(supabase: Client): Promise<FunAward[]> {
  ...
  if (narcissist && narcissist.value > 0) {
    awards.push({
      ...
      blurb: "S'est désigné·e soi-même le plus souvent en 1re place.",
  ...
  if (soulmates) {
    awards.push({
      ...
      blurb: "La paire qui se désigne le plus souvent l'un l'autre.",
  ...
  if (nemesis && nemesis.value.diff > 0) {
    awards.push({
      ...
      blurb: "Vote systématiquement pour la même personne, sans retour.",
  ...
  if (shooterKing && shooterKing.value > 0) {
    awards.push({
      ...
      blurb: "A calé le plus de shooters, toutes éditions confondues.",
